...
//want it to be an array, so wouldn't be a forEach higher order function. seems most likely to use map, because we want an array.
const nums = strNums.map(Number);

console.log(nums);

// Find the sum of the even values
//changed to nums with the filter function instead of strNums because don't want to deal with them as strings anymore; easier to use numbers. Don't have to rewrite a bunch of code, which was initially my plan.
...
//want to return an object after manipulating the strNums array, and also the words "for each" are actually in the problem, so I'm thinking this will probably be a forEach function to solve this one.
const numCount = {};
nums.forEach((num) => {
